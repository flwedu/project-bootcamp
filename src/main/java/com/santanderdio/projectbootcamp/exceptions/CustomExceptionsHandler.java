...
/**
 * Essa classe é responsável por tratar as excessões assim que são lançadas pela
 * Java Runtime, e retornar uma {@code ResponseEntity}.
 * <p>
 * -> O método {@code handleSecurity} é disparado assim que uma excessão do tipo
 * {@link BusinessException} é detectada.
 * <p>
 * -> Após isso, é retornado uma {@link ResponseEntity} com o código <i>422
 * Unprocessable Entity</i> no header.
 * <p>
 * -> No body dessa resposta vai a mensagem, dentro de um
 * objeto {@link ExceptionResponse}. Essa mensagem foi passada pela excessão
 * lançada via <b>throw new BusinessException(mensagem)</b>
 */
@ControllerAdvice
